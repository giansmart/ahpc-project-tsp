...
Node<T>* TSPbranchandbound(std::vector<std::vector<T>> &adjacensyMatrix, PerformanceMetrics &metrics)
{
    ...
    while (!pq.empty())
    {
        ...
        if (min->level == N - 1)
        {
            ...
            metrics.synchronization_time = metrics.total_time - total_computation_time;
        ...
        // MEDIR TIEMPO DE COMPUTACIÓN DEL NODO MAESTRO
        ...
        total_computation_time += comp_elapsed.count();
